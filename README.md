...
### Component Breakdown
* **Orchestration Layer:** Handles request routing (`/stitch/zip` & `/stitch/urls`), payload validation (Pydantic 2.3.0), and API Key authentication.
...
| :--- | :--- | :--- |
| **Core Engine** | Python 3.10-slim | Primary runtime environment and business logic execution. |
| **API Framework** | FastAPI (0.103.1) & Uvicorn (0.23.2) | Asynchronous request handling, routing, and endpoint exposure. |
| **Automation / Driver**| OpenCV Headless (4.8.0) & Numpy (1.24.3) | Low-level image matrix manipulation, greyscale conversion, and MP4 encoding. |
| **Data Layer** | HTTPX (0.25.0) & Pydantic (2.3.0) | Concurrent network I/O for fetching external telemetry arrays and payload validation. |
...
* **High-Throughput Zip Mode (`/v1/video/stitch/zip`):** Designed for bulk payload processing where the client compresses all frames prior to transit. Expects `multipart/form-data` with `fps` (1-120), `width` (320-3840), `height` (240-2160), `mse_threshold` (default 5.0), and `max_idle_ms` (default 10000).
* **Concurrent Fetch Mode (`/v1/video/stitch/urls`):** Executes high-concurrency downloads directly from pre-signed cloud storage URLs bounded by system semaphores (`MAX_DOWNLOAD_CONCURRENCY`). Expects a JSON payload matching the `UrlStitchRequest` schema.
...
The API responds directly with the compiled `.mp4` binary file (`video/mp4` media type) via `FileResponse`. The execution context (telemetry) is calculated internally:
